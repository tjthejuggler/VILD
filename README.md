...
### 2026-09-11T08:55 UTC
- **Reality check content libraries massively expanded** (extensive web research: r/LucidDreaming comprehensive-RC threads, LucidWiki, World of Lucid Dreaming, oneironauts.io, The Lucid Guide / LaBerge's critical state testing):
  - [`TechniqueRepository.kt`](app/src/main/java/com/example/vild/data/TechniqueRepository.kt): added [`EXPANDED_TECHNIQUES`](app/src/main/java/com/example/vild/data/TechniqueRepository.kt:45) — 42 new reality-check ideas (pulse check, coin freeze, finger-pull, skin stretch, ear-pop, faucet test, shadow check, dream-character questioning, two-check pairing, burden-of-proof mindset, and more). Seeded via new [`seedExpandedIfAbsent()`](app/src/main/java/com/example/vild/data/TechniqueRepository.kt:105) using its own `has_seeded_expanded_v2` flag so existing installs receive the additions; exact-text dedup means user-deleted/edited classics are never resurrected. The idea library is now 62 checks.
  - [`RealityCheckRepository.kt`](app/src/main/java/com/example/vild/data/RealityCheckRepository.kt): Today's Reality Check triggers previously started **empty** (only a single fallback line). Added [`SEEDED_TRIGGERS`](app/src/main/java/com/example/vild/data/RealityCheckRepository.kt:17) — 50 daily prompt triggers organised by expert practice: core "am I dreaming" questions, moment-anchored prospective-memory targets (LaBerge state tests: doorways, phone unlocks, mirrors, faucets), false-awakening/waking protocol, dream-sign spotting, physics/perception probes and metacognition prompts. Seeded once via new [`seedIfEmpty()`](app/src/main/java/com/example/vild/data/RealityCheckRepository.kt:119) with its own flag + exact-text dedup.
  - Both repositories' `add()` now silently ignore exact duplicates, so user entries can never double up (the complaint that motivated this work).
  - [`MainViewModel.kt`](app/src/main/java/com/example/vild/MainViewModel.kt:262): calls both seeders in `init`; [`RealityCheckDialog.kt`](app/src/main/java/com/example/vild/ui/realitycheck/RealityCheckDialog.kt:85) explains the shipped starter prompts.
